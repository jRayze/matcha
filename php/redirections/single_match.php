...
if (isset($_SESSION["user_id"])) {
    if (isset($_GET["notif_id"]) && is_numeric($_GET["notif_id"])) {
        ...
        $bdd = get_connection();
        $stmt_notif = $bdd->prepare("SELECT * FROM notif_matches WHERE id=$secure_id AND to_user=$_SESSION[user_id];");
        $stmt_notif->execute();

        if (($query = $stmt_notif->fetch())) {
            $redirect_url = "Location: /usersProfiles/index.php?user_id=".$query["from_user"];

            $stmt_seen = $bdd->prepare("UPDATE notif_matches SET seen=1 WHERE id=$secure_id;");
            $stmt_seen->execute();
        }
    }
} else {
